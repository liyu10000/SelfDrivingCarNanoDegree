...
def compute_precision_recall(det_performance_all, conf_thresh=0.5):
    ...
    pos_negs_all = np.asarray([det_performance[2] for det_performance in det_performance_all])      
    positives = sum(pos_negs_all[:,0])
    true_positives = sum(pos_negs_all[:,1])
    false_negatives = sum(pos_negs_all[:,2])
    false_positives = sum(pos_negs_all[:,3])
...
def pcl_to_bev(lidar_pcl, configs, vis=True):
    ...
    lidar_pcl_cpy[:, 2] -= configs.lim_z[0]
    ...
    _, idx_height_unique = np.unique(lidar_pcl_hei[:, :2], axis=0, return_index=True)
    ...
    _, idx_intensity_unique = np.unique(lidar_pcl_int[:, :2], axis=0, return_index=True)
